...
Adapted from VarunGumma/IndicTransToolkit (without Cython/C compiler dependency on Windows).
"""
...
class IndicProcessor:
    ...
    def _wrap_with_placeholders(self, text: str) -> str:
        ...
        for pattern in patterns:
            ...
            for match in matches:
                ...
                for prefix, suffix in [("<", ">"), ("< ", " >"), ("[", "]"), ("[ ", " ]"), ("[ID ", "]")]:
                    placeholder_entity_map[f"{prefix}ID{serial_no}{suffix}"] = match
                    placeholder_entity_map[f"{prefix}id{serial_no}{suffix}"] = match
    ...
    def _preprocess(
        self, sent: str, src_lang: str, tgt_lang: Optional[str], normalizer, is_target: bool
    ) -> str:
        iso_lang = self._flores_codes.get(src_lang, "hi")
        script_part = src_lang.split("_")[1] if "_" in src_lang else "Deva"
    ...
    def _postprocess(self, sent, lang: str, placeholder_entity_map: Optional[dict] = None) -> str:
        ...
        lang_code, script_code = lang.split("_", 1) if "_" in lang else (lang, "Latn")
    ...
    def preprocess_batch(
        ...
        iterator = tqdm(batch, total=n, desc=f" | > Pre-processing {src_lang}", unit="line") if visualize else batch
